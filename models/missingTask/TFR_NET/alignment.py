...
class CM_ATTN(nn.Module):
    ...
    def get_network(self, self_type='l', layers=-1):
        if self_type in ['l', 'al', 'vl']:
            ...
            embed_dim, attn_dropout, num_heads, position_embedding = self.seq_lens[0], self.attn_dropout, 1, False
        elif self_type == 'a_final':
            embed_dim, attn_dropout, num_heads, position_embedding = self.seq_lens[1], self.attn_dropout, 1, False
        elif self_type == 'v_final':
            embed_dim, attn_dropout, num_heads, position_embedding = self.seq_lens[2], self.attn_dropout, 1, False
        else:
